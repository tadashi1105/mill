...
do_action( 'mill_single_before_entry' );
?>
<article id="post-<?php the_ID(); ?>" <?php post_class( [ 'site-p-entry-card', 'site-c-card' ] ); ?>>
	<?php do_action( 'mill_single_before_entry_header' ); ?>
	<header class="site-p-entry-card__header site-c-card__block">
		<?php
		do_action( 'mill_single_before_entry_title' );
		the_title( '<h1 class="site-p-entry-card__title entry-title">', '</h1>' );
		do_action( 'mill_single_after_entry_title' );
		?>
	</header>
	<?php
	do_action( 'mill_single_before_entry_content' );
	?>
	...
	<?php
	do_action( 'mill_single_after_entry_content' );
	?>
</article>
<?php
do_action( 'mill_single_after_entry' );
