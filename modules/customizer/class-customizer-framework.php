 ...
 * @since Mill 1.0.0
 */
class Mill_Customizer_Framework {
	...
	public function __construct() {
		add_action( 'wp_head', [ $this, 'wp_head' ], 12 );
	}
	...
	public function register_styles( $selectors, $properties, $max_width = '', $min_width = '' ) {
		...
		$this->styles[$key][] = [ $selectors => $properties ];
	}
	...
	public function checkbox( $control_id, $args ) {
		$args = $this->init_field_args( $control_id, $args );
		$args = $this->set_default_sanitize_callback( $args, [ $this, 'sanitize_checkbox' ] );
		$this->customizer->add_setting( $control_id, $args );
		$this->customizer->add_control(
			new WP_Customize_Control(
				$this->customizer,
	...
	public function radio( $control_id, $args ) {
		...
		$args = $this->init_field_args( $control_id, $args );
		$args = $this->set_default_sanitize_callback( $args, [ $this, 'sanitize_choices_' . $control_id ] );
		$this->customizer->add_setting( $control_id, $args );
		$this->customizer->add_control(
			new WP_Customize_Control(
				$this->customizer,
	...
	public function number( $control_id, $args ) {
		$args = $this->init_field_args( $control_id, $args );
		$args = $this->set_default_sanitize_callback( $args, [ $this, 'sanitize_number_' . $control_id ] );
		$this->customizer->add_setting( $control_id, $args );
		$this->customizer->add_control(
			new WP_Customize_Control(
				$this->customizer,
